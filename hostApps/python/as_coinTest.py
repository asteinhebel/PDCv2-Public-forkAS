...
client.runPrint(f"pdcSpad --verbose 4 --pattern {pdcConfig_in['pattern']['pdc3']} --spdc 3 --mode NONE") 
...
COIN_WLEN = int(pdcConfig_in['coincidence']['window_width']) #1   # coincidence windows of X clock cycles
client.runPrint(f"ctlCfg -a COIW -r 0x{COIN_WLEN&0x03FF:04x} -g")

# set the coincidence thresholds
NCH_TH = int(pdcConfig_in['coincidence']['nchannels_threshold']) #3      # number of PDC channel for a coincidence
NUM_BANK = int(pdcConfig_in['coincidence']['nhits'])  #1    # number of hits per PDC
