import os
#import modules.sshClientHelper as sshClientHelper # AS - TEST IN LAB
#from modules.zynqDataTransfer import zynqDataTransfer  # AS - TEST IN LAB


def openZynqConnection(obj, hostCfgName="zcudev"):
    # open a client based on its name in the ssh config file
    #obj.client = sshClientHelper.sshClientFromCfg(hostCfgName=hostCfgName) # AS - TEST IN LAB
    obj.client = 0

def prepZynq(obj):
    #zynq = zynqDataTransfer(sshClientZynq=obj.client)  # AS - TEST IN LAB
    #zynq.hexAppOutPathDefault = os.path.join(HDF5_DATA_DIR, os.path.splitext(obj.scriptName)[0])  # AS - TEST IN LAB
    #zynq.init()  # AS - TEST IN LAB
    obj.zynq = 0
...
def configRegister_pixl(setupObj):
    ...
    #setupObj.client.runPrint(f"pdcCfg -a PIXL -r {PIXL} -g")  # configure pixel register  # AS - TEST IN LAB
    #setupObj.pdcSetting.PIXL = PIXL  # AS - TEST IN LAB
...
def configRegister_time(setupObj):
    ...
    #setupObj.client.runPrint(f"pdcTime --hold {HOLD_TIME} --rech {RECH_TIME} --flag {FLAG_TIME} -g") # AS - TEST IN LAB
    #setupObj.pdcSetting.TIME = setupObj.client.runReturnSplitInt('pdcTime -g') # AS - TEST IN LAB
